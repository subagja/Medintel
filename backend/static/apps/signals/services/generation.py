...
def generate_signal_code() -> str:
    ...
    return f"{prefix}{latest_number + 1:04d}"


def indicator_reference_date(indicator: Indicator):
    if indicator.event_date:
        return indicator.event_date

    evidence = (
        indicator.evidences.select_related("article")
        .order_by("-is_primary_evidence", "created_at")
        .first()
    )

    if evidence is not None:
        timestamp = (
            evidence.article.published_at
            or evidence.article.crawled_at
        )
...
def find_existing_signal(
    ...
    article_ids = list(
        indicator.evidences.values_list(
            "article_id",
            flat=True,
        )
    )
    ...
    if article_ids:
        article_signal = (
            Signal.objects.filter(
                ...
                signal_articles__article_id__in=article_ids,
            )
            .exclude(
                status__in=[
        ...
        if article_signal is not None:
            return article_signal

    reference_date = indicator_reference_date(indicator)
